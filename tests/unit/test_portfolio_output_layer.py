...
class PortfolioOutputLayerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_successful_portfolio_output_from_legitimate_policy_output(self) -> None:
        policy_output = self._build_action_policy_output(impact_score=0.76)

        portfolio_output = self.portfolio_output_service.generate(
            PortfolioOutputRequest(
                ...
                actor_id="portfolio-output-test",
            )
        )

        self.assertEqual(portfolio_output.portfolio_output_status, "ready_for_downstream_use")
        self.assertEqual(portfolio_output.allocation_posture, "bounded_allocation")
        self.assertEqual(portfolio_output.weight_posture, "weighted_allocation")
        self.assertEqual(
            portfolio_output.action_boundary_posture,
            "allocative_non_instructional",
        )

    def test_blocked_portfolio_output_due_to_missing_required_context(self) -> None:
        policy_output = self._build_information_policy_output(impact_score=0.55)
        ...
        self.assertEqual(portfolio_output.portfolio_output_status, "blocked")
        self.assertIn("preference_rank_reference", portfolio_output.missing_context_fields)
    ...
    def test_fallback_template_usage(self) -> None:
        ...
        self.assertEqual(portfolio_output.portfolio_output_status, "fallback_template_applied")
        self.assertEqual(
            portfolio_output.portfolio_output_template_id,
            "portfolio_fallback_generic_output",
        )

    def test_portfolio_output_completeness_and_lineage_mapping(self) -> None:
        policy_output = self._build_information_policy_output(impact_score=0.55)
        ...
        portfolio_output = self.portfolio_output_service.generate(
            PortfolioOutputRequest(
                ...
                portfolio_output_context={
                    "portfolio_summary": "Preserve ranked clarification preference until ambiguity is resolved.",
                    "portfolio_output_reference": "portfolio-output:clarification-rank-001",
        ...
        self.assertEqual(
            portfolio_output.required_context_snapshot["clarification_reference"],
            "clarification-placeholder-001",
        )
        self.assertEqual(portfolio_output.lineage["policy_output_id"], policy_output.policy_output_id)
        self.assertEqual(
            portfolio_output.lineage["portfolio_output_template_id"],
            portfolio_output.portfolio_output_template_id,
        )

    def test_integration_with_lifecycle_authority_router_review_policy_and_portfolio_path(self) -> None:
        ...
        feature = self.feature_registry.register_feature(
            FeatureDefinition(
                ...
                description="Expected revenue delta used to seed portfolio output tests.",
        ...
        orchestrator.record_handoff(
            ...
            reason="Assessment should emit a governed portfolio output after policy output generation.",
            correlation_id=correlation_id,
            actor_id="portfolio-output-test",
            actor_role="case_operator",
        ...
        self.assertEqual(payload["portfolio_output_status"], "ready_for_downstream_use")
        self.assertEqual(payload["portfolio_output_class_id"], "portfolio_bounded_allocation")
        self.assertEqual(
            payload["portfolio_output_action_boundary_posture"],
            "allocative_non_instructional",
        )

    def test_audit_emission_for_portfolio_output_outcomes(self) -> None:
        action_policy_output = self._build_action_policy_output(impact_score=0.76)
        information_policy_output = self._build_information_policy_output(impact_score=0.55)
        fallback_policy_output = self._build_fallback_policy_output()

        self.portfolio_output_service.generate(
            PortfolioOutputRequest(
                policy_output=action_policy_output,
                portfolio_output_class_id="portfolio_bounded_allocation",
                portfolio_author_role="case_operator",
                portfolio_output_context={
                    "portfolio_summary": "Preserve bounded allocation posture for downstream portfolio planning.",
        ...
        self.portfolio_output_service.generate(
            PortfolioOutputRequest(
                policy_output=information_policy_output,
                portfolio_output_class_id="portfolio_ranked_preference",
                portfolio_author_role="case_operator",
                portfolio_output_context={
                    "portfolio_summary": "Preserve ranked clarification preference until ambiguity is resolved."
        ...
        self.assertIn("decision.output.portfolio_output_recorded", event_types)
        self.assertIn("decision.output.portfolio_output_blocked", event_types)
        self.assertIn("decision.output.portfolio_output_ready_for_downstream_use", event_types)
        self.assertIn("decision.output.portfolio_output_missing_context", event_types)
        self.assertIn("decision.output.portfolio_output_fallback_template_applied", event_types)
