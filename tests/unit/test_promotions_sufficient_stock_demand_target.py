...
class SufficientStockDemandTargetTests(unittest.TestCase):
    ...
    def test_stock_constrained_repair_requires_repair_evidence(self) -> None:
        ...
        self.assertEqual(repaired.loc[0, "target_repair_basis"], "REPAIR_POST14_FOLLOWTHROUGH|CEILING_MIN_DEMAND_STOCK")
        self.assertLessEqual(float(repaired.loc[0, "sufficient_stock_demand_units_target"]), 100.0)
        self.assertEqual(float(repaired.loc[0, "target_weight"]), 0.20)

    def test_repair_underallocation_uses_min_demand_stock_ceiling_not_uncapped_demand(self) -> None:
        result = build_sufficient_stock_demand_target_frame(
            _base_row(
                actual_units_sold=0.0,
                stock_basis_units=10.0,
                demand_reference_units=100.0,
                target_underallocation_flag=1,
                target_stockout_flag=0,
                promo_sales_day_count=3.0,
                live_promo_window_days=7.0,
            )
        )
        target = float(result.loc[0, "sufficient_stock_demand_units_target"])
        self.assertEqual(result.loc[0, "target_quality_label"], "STOCK_CONSTRAINED_REPAIRED")
        self.assertEqual(target, 10.0)
        self.assertNotEqual(target, 85.0)
        self.assertIn("REPAIR_UNDERALLOCATION", str(result.loc[0, "target_repair_basis"]))

    def test_repaired_target_never_exceeds_stock_basis_when_stock_exists(self) -> None:
        result = build_sufficient_stock_demand_target_frame(
            _base_row(
                actual_units_sold=1.0,
                stock_basis_units=6.0,
                demand_reference_units=90.0,
                target_underallocation_flag=1,
                target_stockout_flag=0,
                promo_sales_day_count=2.0,
            )
        )
        self.assertLessEqual(float(result.loc[0, "sufficient_stock_demand_units_target"]), 6.0)

    def test_repaired_target_not_below_realized_sales(self) -> None:
        result = build_sufficient_stock_demand_target_frame(
            _base_row(
                actual_units_sold=15.0,
                stock_basis_units=10.0,
                demand_reference_units=90.0,
                target_underallocation_flag=1,
                target_stockout_flag=0,
        ...
        self.assertGreaterEqual(float(result.loc[0, "sufficient_stock_demand_units_target"]), 15.0)
        self.assertIn("REPAIR_CEILING_BELOW_REALIZED_USED_REALIZED", str(result.loc[0, "target_warning"]))

    def test_repaired_target_weight_capped_at_0_20(self) -> None:
        result = build_sufficient_stock_demand_target_frame(
            _base_row(
                actual_units_sold=99.0,
                stock_basis_units=100.0,
        ...
        self.assertEqual(float(result.loc[0, "target_weight"]), 0.20)

    def test_clean_row_weight_unchanged_at_one(self) -> None:
        result = build_sufficient_stock_demand_target_frame(_base_row())
        self.assertEqual(float(result.loc[0, "target_weight"]), 1.0)

    def test_underallocation_without_stock_basis_stays_insufficient(self) -> None:
        result = build_sufficient_stock_demand_target_frame(
            pd.DataFrame(
                {
                    "actual_units_sold": [0.0],
                    "demand_reference_units": [40.0],
                    "target_underallocation_flag": [1],
                    "target_stockout_flag": [0],
                    "promo_sales_day_count": [2.0],
                    "live_promo_window_days": [7.0],
