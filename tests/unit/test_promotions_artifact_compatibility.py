from __future__ import annotations

from pathlib import Path
import json
import sys
import tempfile
import unittest

REPO_ROOT = Path(__file__).resolve().parents[2]
sys.path.append(str(REPO_ROOT / "src"))

from models.promotions.trainer import PromotionModelTrainer  # noqa: E402
from runtime.promotions.artifact_compatibility import (  # noqa: E402
    PromotionArtifactCompatibilityError,
    assert_artifact_compatibility,
    build_artifact_compatibility_result,
)
from runtime.promotions.artifact_locator import (  # noqa: E402
    resolve_model_bundle,
    resolve_training_ready_artifact,
)
from runtime.promotions.config import PromotionArtifactPaths  # noqa: E402
from state.promotions.datasets.dataset_assembler import PromotionDatasetAssembler  # noqa: E402
from state.promotions.feature_engineering import PromotionFeatureEngineer  # noqa: E402
from state.promotions.targets import PromotionTargetEngineer  # noqa: E402
from tests.unit.promotions_test_data import build_repeating_promotions_base_frame  # noqa: E402


def _assemble_dataset(artifact_paths: PromotionArtifactPaths, run_id: str):
    base_frame = build_repeating_promotions_base_frame()
    target_result = PromotionTargetEngineer().engineer(base_frame)
    feature_result = PromotionFeatureEngineer().engineer(target_result.frame)
    return PromotionDatasetAssembler().assemble_training_dataset(
        run_id=run_id,
        base_frame=base_frame,
        target_frame=target_result.frame,
        feature_frame=feature_result.frame,
        target_columns=target_result.target_columns,
        feature_columns=feature_result.feature_columns,
        artifact_paths=artifact_paths,
    )


class PromotionArtifactCompatibilityTests(unittest.TestCase):
    def test_compatibility_passes_for_matching_dataset_and_model_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            artifact_paths = PromotionArtifactPaths(root=Path(temp_dir) / "promotions_artifacts")
            dataset = _assemble_dataset(artifact_paths, run_id="20240201T000000Z")
            PromotionModelTrainer().train(
                run_id="20240201T010000Z",
                dataset=dataset.frame,
                dataset_path=dataset.dataset_path,
                artifact_paths=artifact_paths,
            )

            resolved_dataset = resolve_training_ready_artifact(
                artifact_paths=artifact_paths,
                dataset_path=dataset.dataset_path,
            )
            resolved_model = resolve_model_bundle(
                artifact_paths=artifact_paths,
                model_run_id="20240201T010000Z",
            )
            compatibility = build_artifact_compatibility_result(
                dataset_artifact=resolved_dataset,
                model_bundle=resolved_model,
            )

            self.assertTrue(compatibility.is_compatible)
            self.assertEqual(compatibility.dataset_lineage_relation, "same_training_dataset")
            self.assertEqual(compatibility.reasons, ())

    def test_compatibility_failure_is_clear_and_deterministic(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            artifact_paths = PromotionArtifactPaths(root=Path(temp_dir) / "promotions_artifacts")
            dataset = _assemble_dataset(artifact_paths, run_id="20240201T000000Z")
            PromotionModelTrainer().train(
                run_id="20240201T010000Z",
                dataset=dataset.frame,
                dataset_path=dataset.dataset_path,
                artifact_paths=artifact_paths,
            )
            resolved_model = resolve_model_bundle(
                artifact_paths=artifact_paths,
                model_run_id="20240201T010000Z",
            )
            manifest_path = Path(dataset.manifest_path)
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            model_feature_columns = [
                str(column_name)
                for column_name in resolved_model.inference_schema["feature_columns"]
                if str(column_name).startswith("feature_")
            ]
            manifest["feature_columns"] = [
                str(column_name)
                for column_name in manifest["feature_columns"]
                if str(column_name) != model_feature_columns[0]
            ]
            manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True), encoding="utf-8")

            resolved_dataset = resolve_training_ready_artifact(
                artifact_paths=artifact_paths,
                dataset_path=dataset.dataset_path,
            )

            with self.assertRaises(PromotionArtifactCompatibilityError) as error_context:
                assert_artifact_compatibility(
                    dataset_artifact=resolved_dataset,
                    model_bundle=resolved_model,
                )

            self.assertIn(
                "dataset feature columns do not cover model inference feature columns",
                str(error_context.exception),
            )
