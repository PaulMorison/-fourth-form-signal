...
class PolicyOutputLayerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_successful_policy_output_from_legitimate_recommendation(self) -> None:
        recommendation = self._build_action_recommendation(impact_score=0.76)

        policy_output = self.policy_output_service.generate(
            PolicyOutputRequest(
                ...
                policy_output_context={
                    ...
                    "policy_rationale": "The governed recommendation and review lineage support a bounded policy allowance for downstream preparation.",
                ...
                actor_id="policy-output-test",
            )
        )

        self.assertEqual(policy_output.policy_output_status, "ready_for_downstream_use")
        self.assertEqual(
            policy_output.action_boundary_posture,
            "policy_shaped_non_instructional",
        )
        self.assertEqual(policy_output.bounded_policy_posture, "policy_shaped_allowance")

    def test_blocked_policy_output_due_to_missing_required_context(self) -> None:
        recommendation = self._build_information_recommendation(impact_score=0.55)
        ...
        policy_output = self.policy_output_service.generate(
            PolicyOutputRequest(
                ...
                policy_author_role="case_operator",
                policy_output_context={
                    "policy_summary": "Preserve clarification-first policy posture until stronger downstream meaning is justified."
                },
                correlation_id=str(uuid4()),
                actor_id="policy-output-test",
            )
        ...
        self.assertEqual(policy_output.policy_output_status, "blocked")
        self.assertIn("policy_output_reference", policy_output.missing_context_fields)
    ...
    def test_fallback_template_usage(self) -> None:
        ...
        self.assertEqual(policy_output.policy_output_status, "fallback_template_applied")
        self.assertEqual(
            policy_output.policy_output_template_id,
            "policy_fallback_generic_output",
        )

    def test_policy_output_completeness_and_lineage_mapping(self) -> None:
        recommendation = self._build_information_recommendation(impact_score=0.55)
        ...
        policy_output = self.policy_output_service.generate(
            PolicyOutputRequest(
                ...
                policy_output_context={
                    ...
                    "policy_output_reference": "policy-output:clarification-hold-001",
        ...
        self.assertEqual(
            policy_output.required_context_snapshot["clarification_reference"],
            "clarification-placeholder-001",
        )
        self.assertEqual(policy_output.lineage["recommendation_id"], recommendation.recommendation_id)
        self.assertEqual(
            policy_output.lineage["policy_output_template_id"],
            policy_output.policy_output_template_id,
        )

    def test_integration_with_lifecycle_authority_router_review_and_recommendation_path(self) -> None:
        ...
        feature = self.feature_registry.register_feature(
            FeatureDefinition(
                ...
                description="Expected revenue delta used to seed policy output tests.",
        ...
        orchestrator.record_handoff(
            ...
            reason="Assessment should emit a governed policy output after recommendation generation.",
            correlation_id=correlation_id,
            actor_id="policy-output-test",
            actor_role="case_operator",
        ...
        self.assertEqual(payload["policy_output_status"], "ready_for_downstream_use")
        self.assertEqual(payload["policy_output_class_id"], "policy_shaped_allowance")
        self.assertEqual(
            payload["policy_output_action_boundary_posture"],
            "policy_shaped_non_instructional",
        )

    def test_audit_emission_for_policy_output_outcomes(self) -> None:
        action_recommendation = self._build_action_recommendation(impact_score=0.76)
        information_recommendation = self._build_information_recommendation(impact_score=0.55)
        fallback_recommendation = self._build_fallback_recommendation()

        self.policy_output_service.generate(
            PolicyOutputRequest(
                ...
                policy_output_context={
                    "policy_summary": "Preserve bounded allowance for downstream action preparation.",
                    "policy_output_reference": "policy-output:allowance-001",
                    "policy_rationale": "Recommendation and review lineage justify bounded policy allowance.",
                    "action_boundary_summary": "Policy-shaped allowance only; downstream commitment and instruction remain separate.",
                    "allowance_reference": "allowance:store:001:price-adjustment-window",
                },
                correlation_id=str(uuid4()),
        ...
        self.policy_output_service.generate(
            PolicyOutputRequest(
                recommendation=information_recommendation,
                policy_output_class_id="policy_shaped_information_requirement",
                policy_author_role="case_operator",
                policy_output_context={
                    "policy_summary": "Preserve clarification-first policy posture until ambiguity is resolved."
        ...
        self.assertIn("decision.output.policy_output_recorded", event_types)
        self.assertIn("decision.output.policy_output_blocked", event_types)
        self.assertIn("decision.output.policy_output_ready_for_downstream_use", event_types)
        self.assertIn("decision.output.policy_output_missing_context", event_types)
        self.assertIn("decision.output.policy_output_fallback_template_applied", event_types)
