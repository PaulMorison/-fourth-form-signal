...
PHASE6G_DIR = Path("Diagnostics/phase6g01_dynamic_feature_registry")
...
STORE_772_EXTENDED_EXPORTS = (
    ...
    "PROMO_DAG_STATE_POPULATION.csv",
    "PROMO_DAG_STATE_COVERAGE_SUMMARY.csv",
    "PROMO_ADJACENT_CONFIDENCE_CALIBRATION.csv",
    "PROMO_ADJACENT_POLICY_REVIEW.csv",
    "ACTIVE_LEARNING_TOP_100_REVIEW.xlsx",
    "PROMO_FEATURE_MERGE_OPPORTUNITY_REVIEW.csv",
    "PROMO_SEGMENT_CALIBRATION_ELIGIBILITY_REPAIR.csv",
    "PROMO_PHASE6D_RELEASE_GATE.csv",
    "PROMO_FEATURE_MERGE_PLAN.csv",
    "PROMO_CORE_FRAME_MERGE_AUDIT.csv",
    "PROMO_ATS_EVIDENCE_STRENGTHENING.csv",
...
def _make_run_folder(export_root: Path, run_date: date | None = None, pack_suffix: str = "phase5y_operating_pack") -> Path:
    d = run_date or date.today()
    return export_root / f"{d.isoformat()}_{pack_suffix}"
...
def build_promo_operating_pack(
    *,
    ...
    pack_suffix: str = "phase5y_operating_pack",
) -> dict[str, Any]:
    """Collect current best outputs into an operating pack."""
    root = _export_root(export_root)
    run_folder = _make_run_folder(root, run_date=run_date, pack_suffix=pack_suffix)
...
def write_phase5y_diagnostics(
    *,
    ...
    pack = build_promo_operating_pack(store_number=store_number, export_root=export_root, pack_suffix=pack_suffix)
...
def run_store_772_reporting_export(
    *,
    ...
    phase6g_dir: Path = PHASE6G_DIR,
    store_number: int = 772,
    phase_or_run_id: str = "phase6g01",
    feature_inspection_path: Path | str | None = None,
) -> dict[str, Any]:
    """Export store 772 operating pack on every major run, including Phase 6B–6G audits."""
    ...
    y_result = write_phase5y_diagnostics(
        export_root=export_root,
        diagnostics_dir=diagnostics_dir,
        store_number=store_number,
        pack_suffix="phase6g_dynamic_feature_registry_pack",
    )
    ...
    phase6g = run_phase6g01_dynamic_feature_registry(
        diagnostics_dir=phase6g_dir,
    ...
    phase6d_copies = {
        "PROMO_DAG_STATE_POPULATION.csv": phase6d_dir / "phase6d01_dag_state_population.csv",
        "PROMO_DAG_STATE_COVERAGE_SUMMARY.csv": phase6d_dir / "phase6d01_dag_state_coverage_summary.csv",
        "PROMO_ADJACENT_CONFIDENCE_CALIBRATION.csv": phase6d_dir / "phase6d01_adjacent_confidence_calibration.csv",
        "PROMO_ADJACENT_POLICY_REVIEW.csv": phase6d_dir / "phase6d01_adjacent_policy_review.csv",
        "ACTIVE_LEARNING_TOP_100_REVIEW.xlsx": phase6d_dir / "ACTIVE_LEARNING_TOP_100_REVIEW.xlsx",
        "PROMO_FEATURE_MERGE_OPPORTUNITY_REVIEW.csv": phase6d_dir / "phase6d01_feature_merge_opportunity_review.csv",
        "PROMO_SEGMENT_CALIBRATION_ELIGIBILITY_REPAIR.csv": phase6d_dir / "phase6d01_segment_calibration_eligibility_repair.csv",
        "PROMO_PHASE6D_RELEASE_GATE.csv": phase6d_dir / "phase6d01_release_gate.csv",
    }
    phase6e_copies = {
        "PROMO_FEATURE_MERGE_PLAN.csv": phase6e_dir / "phase6e01_feature_merge_plan.csv",
        "PROMO_CORE_FRAME_MERGE_AUDIT.csv": phase6e_dir / "phase6e01_core_frame_feature_merge_audit.csv",
        "PROMO_ATS_EVIDENCE_STRENGTHENING.csv": phase6e_dir / "phase6e01_ats_evidence_strengthening.csv",
        "PROMO_SEGMENT_CALIBRATION_ELIGIBILITY_AFTER_MERGE.csv": phase6e_dir / "phase6e01_segment_calibration_eligibility_after_merge.csv",
    ...
    phase6g_copies = {
        "PROMO_DYNAMIC_FEATURE_REGISTRY.csv": phase6g_dir / "phase6g01_dynamic_feature_registry.csv",
        "PROMO_LEGACY_SELECTOR_DIFF.csv": phase6g_dir / "phase6g01_legacy_selector_diff.csv",
        "PROMO_BRAIN_VISIBILITY_AFTER_REGISTRY.csv": phase6g_dir / "phase6g01_brain_visibility_after_registry.csv",
        "PROMO_UPLIFT_MODEL_FEATURE_SET.csv": phase6g_dir / "phase6g01_uplift_model_feature_set.csv",
        "PROMO_ECONOMIC_VALUE_MODEL_FEATURE_SET.csv": phase6g_dir / "phase6g01_economic_value_model_feature_set.csv",
        "PROMO_STOCK_EXIT_MODEL_FEATURE_SET.csv": phase6g_dir / "phase6g01_stock_exit_model_feature_set.csv",
        "PROMO_ACTION_CLASSIFIER_FEATURE_SET.csv": phase6g_dir / "phase6g01_action_classifier_feature_set.csv",
        "PROMO_ACTIVE_LEARNING_MODEL_FEATURE_SET.csv": phase6g_dir / "phase6g01_active_learning_model_feature_set.csv",
        "PROMO_MODEL_MATRIX_MANIFEST.csv": phase6g_dir / "phase6g01_model_matrix_manifest.csv",
        "PROMO_SHADOW_MATRIX_READINESS.csv": phase6g_dir / "phase6g01_shadow_matrix_readiness.csv",
        "PROMO_PHASE6G_RELEASE_GATE.csv": phase6g_dir / "phase6g01_release_gate.csv",
    }
    for out_name, src in {**phase6b_copies, **phase6c_copies, **phase6d_copies, **phase6e_copies, **phase6f_copies, **phase6g_copies}.items():
        if src.exists():
    ...
    status = pd.DataFrame([{
        ...
        "release_recommendation": phase6e.get("release_recommendation", RELEASE_RECOMMENDATION),
        "primary_blocker": phase6e.get("primary_blocker", PRIMARY_BLOCKER),
        ...
        "phase6g_dynamic_registry_active_features": phase6g.get("dynamic_registry_active_model_input_features", 0),
        "phase6g_brain_visibility_score_after": phase6g.get("brain_visibility_score_after", 0),
        "phase6g_visibility_status_after": phase6g.get("visibility_status_after", ""),
        "phase6g_models_ready_for_phase6h": phase6g.get("models_ready_for_phase6h_shadow_training", 0),
    }])
    write_phase6f_store_export_status(str(export_folder), phase6f, diagnostics_dir=phase6f_dir)
    write_phase6g_store_export_status(str(export_folder), phase6g, diagnostics_dir=phase6g_dir)
    ...
    phase6c_dir.mkdir(parents=True, exist_ok=True)
    ...
    return {
        ...
        **{k: v for k, v in phase6g.items()},
        "export_folder": str(export_folder),
        "reports_exported": exported,
        "missing_reports": missing,
        "store_reporting_export_status": str(phase6g_dir / "phase6g01_store_reporting_export_status.csv"),
    }
