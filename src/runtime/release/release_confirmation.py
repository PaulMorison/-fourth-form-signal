...
"""Governed runtime release confirmation.

Canon ownership:
- Consumes a governed release-watch-discipline record and explicit release
  confirmation context to decide whether a promoted release is durably
  confirmed, conditionally confirmed, deferred, blocked for missing context,
  rejected for release-confirmation use, or blocked for prohibited overlap.
- Does not own release-watch execution, observation capture, rollback execution,
  monitoring, reopen handling, orchestration meaning, or lifecycle state meaning.
"""
...
from runtime.release.release_confirmation_audit_adapter import (
    ReleaseConfirmationAuditAdapter,
)
from runtime.release.release_registry import ReleaseRegistry
from runtime.release.release_watch_discipline import ReleaseWatchDisciplineRecord

_REQUIRED_COLLECTION_FIELDS = {
    "required_release_confirmation_fields",
    "optional_release_confirmation_fields",
    "required_audit_fields",
    "prohibited_release_confirmation_fields",
    "required_release_confirmation_snapshot",
    "optional_release_confirmation_snapshot",
...
class ReleaseConfirmationRequest:
    release_watch_discipline: ReleaseWatchDisciplineRecord
    release_confirmation_class_id: str
    release_confirmation_author_role: str
    release_confirmation_context: Mapping[str, object]
    correlation_id: str
    actor_id: str


@dataclass(frozen=True)
class ReleaseConfirmationRecord:
    ...
    release_confirmation_author_role: str
    release_confirmation_author_id: str
    ...
    required_release_confirmation_snapshot: Mapping[str, str]
    optional_release_confirmation_snapshot: Mapping[str, str]
    ...
    release_confirmation_prerequisite_reference: str | None = None
    outstanding_release_confirmation_prerequisites: tuple[str, ...] = ()
    missing_release_confirmation_fields: tuple[str, ...] = ()
    prohibited_release_confirmation_fields_present: tuple[str, ...] = ()
    ...
    def to_transport_context(self) -> dict[str, object]:
        context: dict[str, object] = {
            ...
            "release_confirmation_judgment": self.release_confirmation_judgment,
            ...
            "release_confirmation_threshold_evidence_reference": (
                self.release_confirmation_threshold_evidence_reference
            ),
            "release_confirmation_authority_reference": (
                self.release_confirmation_authority_reference
            ),
        }
        ...
        if self.release_confirmation_prerequisite_reference is not None:
            context["release_confirmation_prerequisite_reference"] = (
                self.release_confirmation_prerequisite_reference
            )
        if self.outstanding_release_confirmation_prerequisites:
            context["outstanding_release_confirmation_prerequisites"] = list(
                self.outstanding_release_confirmation_prerequisites
            )
        return context


class ReleaseConfirmation:
    """Builds release-confirmation records from release-watch-discipline records."""
    ...
    def __init__(
        ...
        *,
        ...
        self._release_confirmation_audit_adapter = release_confirmation_audit_adapter
    ...
    def generate(
        ...
        template, fallback_template_used = (
            self._release_registry.resolve_release_confirmation_template(
                semantic_scope=request.release_watch_discipline.semantic_scope,
                release_watch_discipline_class_id=(
                    request.release_watch_discipline.release_watch_discipline_class_id
                ),
                release_confirmation_class_id=request.release_confirmation_class_id,
                route_name=request.release_watch_discipline.lineage.get("route_name"),
            )
        )
        class_definition = self._release_registry.get_release_confirmation_class(
            request.release_confirmation_class_id
        )
        combined_context = self._combined_context(request, class_definition)
        (
            required_release_confirmation_snapshot,
            missing_release_confirmation_fields,
        ) = self._snapshot_required_fields(
            template.required_release_confirmation_fields,
            combined_context,
        )
        ...
        all_missing_fields = tuple(
            dict.fromkeys(missing_release_confirmation_fields + missing_audit_fields)
        )
        optional_release_confirmation_snapshot = self._snapshot_optional_fields(
            template.optional_release_confirmation_fields,
            combined_context,
        )
        prohibited_release_confirmation_fields_present = (
            self._prohibited_release_confirmation_fields_present(
                class_definition.prohibited_release_confirmation_fields,
                request.release_confirmation_context,
            )
        )
        release_confirmation_status = self._release_confirmation_status(
            release_watch_discipline=request.release_watch_discipline,
            class_definition=class_definition,
            combined_context=combined_context,
            missing_release_confirmation_fields=all_missing_fields,
            prohibited_release_confirmation_fields_present=(
                prohibited_release_confirmation_fields_present
            ),
            fallback_template_used=fallback_template_used,
        )
        release_confirmation_outcome = self._release_confirmation_outcome(
            template=template,
            release_confirmation_status=release_confirmation_status,
        )
        reason = self._reason(
            release_watch_discipline=request.release_watch_discipline,
            release_confirmation_status=release_confirmation_status,
            release_confirmation_class_id=(
                class_definition.release_confirmation_class_id
            ),
            missing_release_confirmation_fields=all_missing_fields,
            prohibited_release_confirmation_fields_present=(
                prohibited_release_confirmation_fields_present
            ...
            class_definition=class_definition,
            combined_context=combined_context,
            release_confirmation_outcome=release_confirmation_outcome,
        )
        lineage = self._lineage(request, class_definition, template)

        upstream = request.release_watch_discipline
        record = ReleaseConfirmationRecord(
            release_confirmation_id=str(uuid4()),
            release_confirmation_status=release_confirmation_status,
            reason=reason,
            release_confirmation_class_id=(
                class_definition.release_confirmation_class_id
            ...
            release_confirmation_template_id=(
                template.release_confirmation_template_id
            ),
            release_confirmation_outcome=release_confirmation_outcome,
            release_confirmation_judgment=self._field_value_or_placeholder(
                field_name="release_confirmation_judgment",
                snapshot=required_release_confirmation_snapshot,
                context=combined_context,
            ),
            release_confirmation_threshold_evidence_reference=(
                self._field_value_or_placeholder(
                    field_name="release_confirmation_threshold_evidence_reference",
                    snapshot=required_release_confirmation_snapshot,
                    context=combined_context,
                )
            ),
            release_confirmation_authority_reference=(
                self._field_value_or_placeholder(
                    field_name="release_confirmation_authority_reference",
                    snapshot=required_release_confirmation_snapshot,
                    context=combined_context,
                )
            ),
            ...
            release_confirmation_author_role=(
                request.release_confirmation_author_role
            ),
            release_confirmation_author_id=request.actor_id,
            required_release_confirmation_fields=(
                template.required_release_confirmation_fields
            ),
            optional_release_confirmation_fields=(
            ...
            release_confirmation_prerequisite_reference=combined_context.get(
                "release_confirmation_prerequisite_reference"
            ),
            outstanding_release_confirmation_prerequisites=(
                combined_context["outstanding_release_confirmation_prerequisites"]
            ),
            missing_release_confirmation_fields=all_missing_fields,
            prohibited_release_confirmation_fields_present=(
                prohibited_release_confirmation_fields_present
        ...
        self._release_confirmation_audit_adapter.record_release_confirmation(
            record,
            request=request,
        )
        return record
    ...
    def _combined_context(
        ...
        upstream = request.release_watch_discipline
        context = dict(upstream.required_release_watch_discipline_snapshot)
        context.update(upstream.optional_release_watch_discipline_snapshot)
        context.update(upstream.required_audit_snapshot)
        context.update(dict(request.release_confirmation_context))
        context.update(
            {
                "release_confirmation_class_id": (
                    class_definition.release_confirmation_class_id
                ...
                "release_watch_discipline_status": (
                    upstream.release_watch_discipline_status
                ),
                "release_watch_discipline_class_id": (
                ...
                "release_watch_discipline_outcome": (
                    upstream.release_watch_discipline_outcome
                ),
                ...
                or upstream.semantic_scope,
                "release_confirmation_judgment": self._optional_text(
                    context.get("release_confirmation_judgment")
                ),
                "release_confirmation_threshold_evidence_reference": (
                    self._optional_text(
                        context.get(
                ...
                "release_confirmation_authority_reference": self._optional_text(
                    context.get("release_confirmation_authority_reference")
                ),
                ...
                "release_confirmation_prerequisite_reference": self._optional_text(
                    context.get("release_confirmation_prerequisite_reference")
                ),
                "outstanding_release_confirmation_prerequisites": (
    ...
    def _prohibited_release_confirmation_fields_present(
        self,
    ...
    def _release_confirmation_status(
        self,
        *,
        release_watch_discipline: ReleaseWatchDisciplineRecord,
        class_definition,
        ...
        if prohibited_release_confirmation_fields_present:
            return "prohibited_overlap_blocked"
        if missing_release_confirmation_fields:
            return "blocked_missing_context"
        if (
            release_watch_discipline.release_watch_discipline_status
            not in class_definition.allowed_release_watch_discipline_statuses
        ):
            return "rejected_for_release_confirmation_use"
        if (
            release_watch_discipline.release_watch_discipline_outcome
            not in class_definition.allowed_release_watch_discipline_outcomes
        ):
            return "rejected_for_release_confirmation_use"
        if self._has_release_confirmation_restrictions(combined_context) and not (
            class_definition.allow_release_confirmation_restrictions
        ):
            return "rejected_for_release_confirmation_use"
        if self._has_release_confirmation_prerequisite_deferral(
            combined_context
        ) and not (class_definition.allow_release_confirmation_prerequisite_deferral):
            return "rejected_for_release_confirmation_use"
        return (
            "fallback_template_applied"
            if fallback_template_used
            else "release_confirmation_judged"
        )

    def _release_confirmation_outcome(
        self,
        *,
        template,
        release_confirmation_status: str,
    ) -> str:
        if release_confirmation_status in {
            "blocked_missing_context",
            "rejected_for_release_confirmation_use",
            "prohibited_overlap_blocked",
        }:
            return release_confirmation_status
        return template.release_confirmation_outcome
    ...
    def _reason(
        ...
        *,
        ...
        if release_confirmation_status == "blocked_missing_context":
            return (
                "Release confirmation is blocked because governed release "
                "confirmation context is missing required fields: "
                + ", ".join(missing_release_confirmation_fields)
                + "."
            )
        if release_confirmation_status == "prohibited_overlap_blocked":
            return (
                "Release confirmation is blocked because context overlaps "
                "watch execution, observation capture, rollback execution, "
                "monitoring, reopen, or orchestration fields: "
                + ", ".join(prohibited_release_confirmation_fields_present)
                + "."
            )
        if release_confirmation_status == "rejected_for_release_confirmation_use":
            if (
                release_watch_discipline.release_watch_discipline_status
                not in class_definition.allowed_release_watch_discipline_statuses
            ):
                return (
                    "Release confirmation rejects release-watch-discipline status '"
                    f"{release_watch_discipline.release_watch_discipline_status}' "
                    f"for class '{release_confirmation_class_id}'."
                )
            if (
                release_watch_discipline.release_watch_discipline_outcome
                not in class_definition.allowed_release_watch_discipline_outcomes
                ...
                return (
                    "Release confirmation rejects release-watch-discipline outcome '"
                    f"{release_watch_discipline.release_watch_discipline_outcome}' "
                    f"for class '{release_confirmation_class_id}'."
                )
            if self._has_release_confirmation_restrictions(combined_context) and not (
                class_definition.allow_release_confirmation_restrictions
                ...
                return (
                    "Release confirmation rejects restricted production exposure "
                    f"for class '{release_confirmation_class_id}'."
                )
            if self._has_release_confirmation_prerequisite_deferral(
                combined_context
                ...
                return (
                    "Release confirmation rejects deferred confirmation evidence "
                    f"for class '{release_confirmation_class_id}'."
                )
        if release_confirmation_outcome == "confirmed_for_broader_trusted_production_use":
            return (
                "Release confirmation threshold evidence meets the governed "
                "confirmation threshold during the watch window and broader "
                "trusted production use remains justified."
            )
        if (
            ...
            return (
                "Release confirmation threshold evidence supports only bounded "
                "production use, so narrowed exposure remains preserved."
            )
        return (
            "Release confirmation is deferred until explicit threshold evidence "
            "and confirmation authority are complete."
        )
    ...
    def _lineage(
        ...
        lineage = dict(request.release_watch_discipline.lineage)
        lineage.update(
            {
                "release_watch_discipline_id": (
                    request.release_watch_discipline.release_watch_discipline_id
                ),
                "release_confirmation_class_id": (
                    class_definition.release_confirmation_class_id
                ),
    ...
    def _has_release_confirmation_restrictions(
        self,
        context: Mapping[str, object],
    ) -> bool:
        return any(
    ...
    def _has_release_confirmation_prerequisite_deferral(
        self,
        context: Mapping[str, object],
    ) -> bool:
        return any(
